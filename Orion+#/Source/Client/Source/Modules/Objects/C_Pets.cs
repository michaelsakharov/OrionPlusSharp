            ...
            if (C_Types.Player[index].Pet.Moving == (byte)Enums.MovementType.Walking)
            {
                switch (C_Types.Player[index].Pet.Dir)
                {
                    ...
                    case (int)Enums.DirectionType.UpLeft:
                        C_Types.Player[index].Pet.YOffset = C_Types.Player[index].Pet.YOffset - C_Constants.WalkSpeed;
                        if (C_Types.Player[index].Pet.YOffset < 0) { C_Types.Player[index].Pet.YOffset = 0; }
                        C_Types.Player[index].Pet.XOffset = C_Types.Player[index].Pet.XOffset - C_Constants.WalkSpeed;
                        if (C_Types.Player[index].Pet.XOffset < 0) { C_Types.Player[index].Pet.XOffset = 0; }
                        break;
                ...
                {
                    if (C_Types.Player[index].Pet.Dir == (int)Enums.DirectionType.Right || C_Types.Player[index].Pet.Dir == (int)Enums.DirectionType.Down || C_Types.Player[index].Pet.Dir == (int)Enums.DirectionType.DownLeft || C_Types.Player[index].Pet.Dir == (int)Enums.DirectionType.DownRight)
                    {
...
#region Drawing
		...
		internal static void DrawPet(int index)
		{
			...
			srcrec = new Rectangle((int)((anim) * ((double) C_Graphics.CharacterGfxInfo[sprite].Width / 4)), (int)(spriteleft * ((double) C_Graphics.CharacterGfxInfo[sprite].Height / 4)), (int)((double) C_Graphics.CharacterGfxInfo[sprite].Width / 4), (int)((double) C_Graphics.CharacterGfxInfo[sprite].Height / 4));
			
			// Calculate the X
			x = (int)(C_Types.Player[index].Pet.X * C_Constants.PicX + C_Types.Player[index].Pet.XOffset - (((double) C_Graphics.CharacterGfxInfo[sprite].Width / 4 - 32) / 2));
			
			// Is the player's height more than 32..
			if (((double) C_Graphics.CharacterGfxInfo[sprite].Height / 4) > 32)
			{
				// Create a 32 pixel offset for larger sprites
				y = (int)(C_Types.Player[index].Pet.Y * C_Constants.PicY + C_Types.Player[index].Pet.YOffset - (((double) C_Graphics.CharacterGfxInfo[sprite].Width / 4) - 32));
			}
			else
			{
				// Proceed as normal
		...
		internal static void DrawPlayerPetName(int index)
		{
			...
			// calc pos
			textX = (int)(C_Graphics.ConvertMapX(C_Types.Player[index].Pet.X * C_Constants.PicX) + C_Types.Player[index].Pet.XOffset + (C_Constants.PicX / 2) - (double) C_Text.GetTextWidth(name) / 2);
			...
			else
			{
				...
				textY = (int)(C_Graphics.ConvertMapY(C_Types.Player[index].Pet.Y * C_Constants.PicY) + C_Types.Player[index].Pet.YOffset - ((double) C_Graphics.CharacterGfxInfo[Pet[C_Types.Player[index].Pet.Num].Sprite].Height / 4) + 16);
			}
		...
		public static void DrawPetBar()
		{
			...
			else
			{
				...
				for (var i = 1; i <= 4; i++)
				{
					skillnum = (int)(C_Types.Player[C_Variables.Myindex].Pet.Skill[(int) i]);
		...
		public static void DrawPetStats()
		{
			...
			rec.Y = 0;
			rec.Height = (int)((double) C_Graphics.CharacterGfxInfo[sprite].Height / 4);
			rec.X = 0;
			rec.Width = (int)((double) C_Graphics.CharacterGfxInfo[sprite].Width / 4);
			...
			C_Graphics.RenderSprite(C_Graphics.CharacterSprite[sprite], C_Graphics.GameWindow, C_UpdateUI.PetStatX + 10, (int)(C_UpdateUI.PetStatY + 10 + ((double) C_Graphics.PetStatsGfxInfo.Height / 4) - ((double) rec.Height / 2)), rec.X, rec.Y, rec.Width, rec.Height);

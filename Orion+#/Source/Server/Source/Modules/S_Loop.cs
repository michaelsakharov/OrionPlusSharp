                ...
                if (modTypes.PlayersOnMap[mapNum] == 1)
                {
                    ...
                    for (x = 1; x <= Constants.MAX_MAP_NPCS; x++)
                    {
                        ...
                        else
                        {
                            ...
                            if (modTypes.Map[mapNum].Npc[x] > 0 && modTypes.MapNpc[mapNum].Npc[x].Num > 0)
                            {
                                var tx = modTypes.MapNpc[mapNum].Npc[x].X;
                                var ty = modTypes.MapNpc[mapNum].Npc[x].Y;
                                var mx = modTypes.Map[mapNum].MaxX;
                                var my = modTypes.Map[mapNum].MaxY;

                                if (tx < 0 || tx > mx || ty < 0 || ty > my)
                                {
                                    // NPC is outside the map? Force him back into a new place!
                                    modTypes.MapNpc[mapNum].Npc[x].X = (byte)S_GameLogic.Random(0, mx);
                                    modTypes.MapNpc[mapNum].Npc[x].Y = (byte)S_GameLogic.Random(0, my);
                                }
                                    ...
                                    if (Types.Npc[npcNum].Behaviour != (int)NpcBehavior.ShopKeeper && Types.Npc[npcNum].Behaviour != (int)NpcBehavior.Quest)
                                    {
                                        ...
                                        if (targetVerify)
                                        {
                                            ...
                                            if (modPathfinding.mapMatrix[mapNum].created)
                                            {
                                                ...
                                                {
                                                    // Target has moved
                                                    modTypes.MapNpc[mapNum].Npc[x].hasPath = modPathfinding.APlus(mapMartixNum, modTypes.MapNpc[mapNum].Npc[x].X, modTypes.MapNpc[mapNum].Npc[x].Y, targetX, targetY, modPathfinding.eCell.Void, ref modTypes.MapNpc[mapNum].Npc[x].arPath);

...
namespace Engine
{
    internal static class S_Projectiles
    {
        ...
        internal static float Engine_GetAngle(int CenterX, int CenterY, int targetX, int targetY)
        {
            ...
            // Calculate Side C
            SideC = (float)Math.Sqrt(Math.Pow(Math.Abs(targetX - CenterX), 2) + Math.Pow(Math.Abs(targetY - CenterY), 2));

            // Side B = CenterY

            // Calculate Side A
            SideA = (float)Math.Sqrt(Math.Pow(Math.Abs(targetX - CenterX), 2) + Math.Pow(targetY, 2));

            // Calculate the angle
            Engine_GetAngle = (float)(Math.Pow(SideA, 2) - Math.Pow(CenterY, 2) - Math.Pow(SideC, 2)) / (CenterY * SideC * -2);
            Engine_GetAngle = (float)((Math.Atan(-Engine_GetAngle / Math.Sqrt(-Engine_GetAngle * Engine_GetAngle + 1)) + 1.5708) * 57.29583);

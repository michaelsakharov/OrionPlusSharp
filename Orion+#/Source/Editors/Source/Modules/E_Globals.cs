...
namespace Engine
{
	sealed class E_Globals
	{
		...
		internal static Color[] TilesetsClr;
		...
		internal static bool[] Item_Changed = new bool[Constants.MAX_ITEMS + 1];
		
		internal static bool[] NPC_Changed = new bool[Constants.MAX_NPCS + 1];
		internal static bool[] Resource_Changed = new bool[Constants.MAX_NPCS + 1];
		internal static bool[] Animation_Changed = new bool[Constants.MAX_ANIMATIONS + 1];
		internal static bool[] Skill_Changed = new bool[Constants.MAX_SKILLS + 1];
		internal static bool[] Shop_Changed = new bool[Constants.MAX_SHOPS + 1];
		...
		// Map constants
		internal static byte SCREEN_MAPX = (byte) 35;
		
		internal static byte SCREEN_MAPY = (byte) 26;
		...
		internal static int HalfX = System.Convert.ToInt32((((byte) 35 + 1) / 2) * PIC_X);
		internal static int HalfY = System.Convert.ToInt32((((byte) 26 + 1) / 2) * PIC_Y);
		internal static int ScreenX = ((byte) 35 + 1) * PIC_X;
		internal static int ScreenY = ((byte) 26 + 1) * PIC_Y;
